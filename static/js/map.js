...
function Commutes(configuration) {
  ...
  function initCommutesModal() {
    ...
    document.getElementById('gasStationButton').addEventListener('click', function() {
      findNearbyPlaces({ lat: 37.773345964182795, lng:  -122.48529082191631 }, 'gas_station');
    });
    
    document.getElementById('restaurantButton').addEventListener('click', function() {
      findNearbyPlaces({ lat: 37.773345964182795, lng:  -122.48529082191631 }, 'restaurant');
    });
    
    function findNearbyPlaces(location, type) {
      const service = new google.maps.places.PlacesService(commutesMap);
      ...
      service.nearbySearch({
          ...
          if (status === google.maps.places.PlacesServiceStatus.OK) {
              let placesHtml = '';
              results.slice(0, 3).forEach(place => { // Limit to 3 results
                  placesHtml += `
                      <div class="place">
                          <h4>${place.name}</h4>
              ...
              document.getElementById('nearby-info-container').innerHTML = placesHtml;
          } else {
              document.getElementById('nearby-info-container').innerHTML = '<p>No places found or error occurred.</p>';
...
function generateDestinationTemplate(destination) {
    ...
    return `
      ...
      <!-- Existing destination-controls code -->
      <div class="destination-controls">
        <!-- ... -->
      </div>`;
  }
